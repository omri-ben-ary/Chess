...
/** copy function of game id for map **/
static MapKeyElement copyGameId(MapKeyElement game_id);

/** copy function of game data for map **/
static MapDataElement copyGameData(MapDataElement data);

/** free function of game id for map**/
static void freeGameId(MapKeyElement game_id);

/** free function of game data for map**/
static void freeGameData(MapKeyElement data);

/** compare game id function for map**/
static int compareGameId(MapKeyElement game_id1, MapKeyElement game_id2);

/** used in gameTableSumUpPoints in order to find a player id in the given player_table, if not found return -1 **/
static int gameTableFindPlayerIndex(int** player_table, int player_id, int table_size);

/** used in gameTableSumUpPoints in order to calculate players added points in each game**/
