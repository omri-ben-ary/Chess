...
/**returns true if location name entered is valid, false otherwise **/
static bool chessLocationNameCheck(const char* name);
/** returns chessResult according to the entered parameters, if the input is valid returns CHESS_SUCCESS **/
...
/**  returns ChessResult according to the entered parameters, if the input was valid it returns CHESS_SUCCESS
chess must not be NULL */
static ChessResult chessAddGameParametersCheck(ChessSystem chess, int tournament_id, int first_player,
                                               int second_player, Winner winner, int play_time);
/** converts winner enum to gameResult enum Which is used in inner ADT's**/
static GameResult chessAddGameCheckForWinner(Winner winner);
/** if a memory allocation failed whilst trying to update players map this
 * function deleted allocated game in tournament map**/
...
/** remove the player from all the tournament's that he was part of and not haven't finished .
chess must not be NULL */
...
/**manages the whole player removal (from the system) process
chess must not be NULL */
...
/** returns the average play time of a player in the all the games he was part of and he kept memory of in his stats.
chess must not be NULL */
...
/** updates the player stats according to his performances in the system,
returns CHESS_SUCCESS when finish without issues or the issue if one was found.
chess must not be NULL */
static ChessResult chessUpdatePlayerStats(ChessSystem chess);

/** updates the player stats with a specific game result.
...
/** performing swap method between two doubles. */
static void chessSwapDoubles(double* x, double* y);

/** performing bubble to the the player rankings array, return 1 if at least one swap was performed */
static int chessBubblePlayersRankings(double a[], int n);

/** performing bubble sort method to the the player rankings array */
static void chessBubbleSortPlayersRankings(double a[], int n);

/** returns the sorted array of player rankings
players must be different than NULL*/
static double* chessCalculateAndSortRankings(Players players, int n);

/**returns CHESS_SUCCESS if the print process of the ranking list to the file succeed, else returns the suited error.
players must be different than NULL*/
...
ChessResult chessSaveTournamentStatistics (ChessSystem chess, char* path_file)
{
    ...
    TournamentError check_result = tournamentTableGetStats(chess->tournament_table,path_file);
