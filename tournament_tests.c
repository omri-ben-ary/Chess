...
void runTournamentsTest()
{
    ...
    assert(tournamentTableAddOrEditTournament(tournament_table, real_id, tournament_data ) == TOURNAMENT_SUCCESS);
    assert(tournamentTableContains(tournament_table, real_id) == true);
    assert(tournamentTableDeleteTournament(tournament_table, real_id) == TOURNAMENT_SUCCESS);
    int ids[TEST_LEN];
    for (int i=1; i<TEST_LEN+1; i++)
    {
        ids[i-1] = i;
        assert(tournamentTableAddOrEditTournament(tournament_table, ids[i-1], tournament_data ) == TOURNAMENT_SUCCESS);
    }
    ...
    assert(tournamentTableRemovePlayerInGame(tournament_table,9,5,5) == TOURNAMENT_SUCCESS);
    ...
    assert(tournamentTableRemovePlayerInGame(tournament_table,8,5,6) == TOURNAMENT_SUCCESS);
    ...
    assert(tournamentTableRemovePlayerInGame(tournament_table,7,1,1) == TOURNAMENT_SUCCESS);
