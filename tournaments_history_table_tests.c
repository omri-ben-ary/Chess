...
void test_of_tournament_history_table()
{
    ...
    printf("is tournament 1 contained: %d (shouldn't be)\n", (MAP_SUCCESS == tournamentHistoryTableContain(my_tournaments_history_table, tournament1_id)));
    printf("put tournament 1 result: %d\n",tournamentHistoryTablePut(my_tournaments_history_table, tournament1_id, tournament1_stats));
    ...
    printf("put tournament 2 result: %d\n",tournamentHistoryTablePut(my_tournaments_history_table, tournament2_id, tournament2_stats));

    TournamentStats tournament3_stats = tournamentStatsCreate(3);
    int tournament3_id = 21236666; // what will happen when same id
    printf("put tournament 3 result: %d\n",tournamentHistoryTablePut(my_tournaments_history_table, tournament3_id, tournament3_stats));

    TournamentStats tournament4_stats = tournamentStatsCreate(3);
    int tournament4_id = 21235;
    printf("put tournament 4 result: %d\n",tournamentHistoryTablePut(my_tournaments_history_table, tournament4_id, tournament4_stats));
    printf("is tournament 1 contained: d (should be): %d\n", tournamentHistoryTableContain(my_tournaments_history_table, tournament2_id));
    tournamentHistoryTableRemove(my_tournaments_history_table, tournament2_id);
    printf("is tournament 1 contained: d (shouldn't be): %d\n", tournamentHistoryTableContain(my_tournaments_history_table, tournament2_id));

    tournamentHistoryTableAddGameStats(my_tournaments_history_table,tournament4_id, 123456);
    tournamentHistoryTableAddGameStats(my_tournaments_history_table,tournament4_id, 123457);
    tournamentHistoryTableAddGameStats(my_tournaments_history_table,tournament1_id, 111111);

    MAP_FOREACH (int * ,id, my_tournaments_history_table)
    {
        TournamentStats current_tournament = tournamentHistoryTableGet(my_tournaments_history_table,*id);
        printf("tournament id: %d, max games allowed: %d\n", *id, tournamentStatsGetMaxGamesAllowed(current_tournament));
        free(id);
    }
